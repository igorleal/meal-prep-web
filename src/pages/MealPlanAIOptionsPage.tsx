...
export default function MealPlanAIOptionsPage() {
  ...
  const handleSubmit = () => {
    ...
    const pendingRequestStr = sessionStorage.getItem('pendingMealPlanRequest')
    ...
    const planName = pendingData.planName

    // Build the full AI request
    const request: GenerateReceitAIPlanRequest = {
      name: planName,
      focusAreas: aiFormData.focusAreas
        ? Object.entries(aiFormData.focusAreas).reduce(
        ...
        : {},
      goal: aiFormData.macros || { calories: null, protein: null, carbs: null, fats: null },
      mustHaves: aiFormData.mustHaves,
      exclusions: aiFormData.exclusions,
      restrictions: aiFormData.restrictions,
      mealsPerDay: aiFormData.mealsPerDay ?? 1,
      days: aiFormData.days ?? 5,
    }
      ...
      <div className="fixed bottom-0 left-0 right-0 md:left-64 border-t border-border-light dark:border-border-dark bg-surface-light/95 dark:bg-background-dark/95 backdrop-blur-sm p-6 z-10 h-[92px]">
        <div className="flex justify-between items-center gap-4 h-full">
          <Button
            variant="secondary"
            icon="arrow_back"
            onClick={() => navigate('/meal-plans/source')}
          >
